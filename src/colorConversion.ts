...
export function compute_max_saturation(a: number, b: number) {
...
export function find_cusp(a: number, b: number) {
...
export function get_ST_mid(a_: number, b_: number) {
  const S =
    0.11516993 +
    1 /
      (+7.4477897 +
...
export function get_Cs(L: number, a_: number, b_: number) {
...
export function hex_to_rgb(hex: string) {
  ...
  if (hex.match(/^([0-9a-f]{1})$/i)) {
    let a = (parseInt(hex.substring(0), 16) / 15) * 255;
    return [a, a, a];
  }
